...
def fix_dataset_configs_geo(repo_path: Path) -> str:
    ...
    tracts_file = (
        repo_path / "datasets" / "modeled" / "dgen" / "dimensions" / "geography.csv"
    )
    mapping_file = (
        repo_path
        / "datasets"
...
def fix_dataset_configs_model_year(repo_path: Path) -> str:
    ...
    dataset_model_years_file = (
        repo_path / "datasets" / "modeled" / "dgen" / "dimensions" / "model_years.csv"
    )
    mapping_file = (
        repo_path
        / "datasets"
...
def fix_dgen_profiles(dst_repo_path: Path) -> None:
    dataset_config_file = (
        dst_repo_path / "datasets" / "modeled" / "dgen" / "dataset.json5"
    )
    ...
    profiles_file = (
        dst_repo_path / "datasets" / "modeled" / "dgen" / "dimensions" / "profiles.csv"
    )
...
def make_dgen_capacities(dst_repo_path: Path) -> None:
    profiles_path = dst_repo_path / "datasets" / "modeled" / "dgen"
